...
export default function AddTranscript(props){
    ...
    const handleSubmit = (event) => {
        ...
        if(isTranscriptValid){
            mockFetch('http://127.0.0.1:5000/transcripts', {
                method: 'POST',
                headers: {
    ...
    // Function to open a (simulated) audio stream and redirect to the recording page
    const openAudioStream = () => {
        mockFetch('http://127.0.0.1:5000/openAudioStream', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        .then(data => {
            console.log(data);
            // Redirect to the recording page with the (fake) audio stream ID
            navigate(`${DEMO_BASE}/Recording/${data.audioStreamID}?port=${data.port}`);
        })
        .catch(error =>{
            console.error(error);
        });
