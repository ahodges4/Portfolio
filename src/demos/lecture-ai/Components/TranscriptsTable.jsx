...
export default function TranscriptsTable(props){
    ...
    const handleClick = () => {
        ...
        // Send a request to the mock API to generate questions for the selected transcript
        mockFetch(`http://127.0.0.1:5000/generateTranscriptQuestions/${document.querySelector('input[name="selectedRow"]:checked').value}`, {
            method: 'POST',
            headers: {
    ...
    const handleDelete = (id) => {
        ...
        if(lastDelete === null){
            ...
            mockFetch(`http://127.0.0.1:5000/transcripts/${id}`, {
            method: 'DELETE'
            })
            .then(response => response.json())
            .then(data => {
